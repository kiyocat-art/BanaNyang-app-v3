...
import { GenerationTask, GeneratedMedia, GenerationBatch, MonthlyCredit, ModelName, GenerationParams } from '../types';
...
export const useImageGeneration = ({
    ...
    useEffect(() => {
        const processNextInQueue = async () => {
            ...
            // FIX: Check cost against available credit before starting generation.
            if (monthlyCredit.current < COST_PER_IMAGE && !userAcknowledgedPaidUsage) {
            ...
            try {
                if (task.taskType === 'image') {
                    ...
                    // FIX: Pass `task.numImages` to the image processing function.
                    const resultSrcs = await processCharacterImage(
                        ...
                        task.selectedPalette, task.numPaletteColors, task.isAutoColorizeSketch, task.numImages
                    );
                    ...
                    const newImages: GeneratedMedia[] = resultSrcs.map((src, index) => ({
                      id: `${jobId}-${Date.now()}-${index}`, src: src, type: 'image', view: task.cameraView, generationParams
                    }));
                    ...
                    if (newImages.length > 0) {
                        ...
                        const originalImage = useCanvasStore.getState().boardImages.find(img => img.role === 'original');
                        if (canvasRect) {
                            addImagesToCenter(newImages, canvasRect, originalImage?.id);
                        }
                        setCurrentPage(1);
