...
import {
  ...
  ScissorsIcon, LandscapeIcon, LassoIcon
} from '../../../components/icons';

interface SelectionBarProps {
    onEdit: (imageId: string) => void;
    onMasking: (imageId: string) => void;
...
export const SelectionBar: React.FC<SelectionBarProps> = ({ onEdit, onMasking, onZoom, onDelete, onDownload, language }) => {
    ...
    const actionButtons = [
        ...
        { key: 'edit', onClick: () => onEdit(selectedImage.id), icon: <ScissorsIcon className="w-5 h-5" />, tooltip: t('tooltip.edit', language) },
        { key: 'lasso', onClick: () => onMasking(selectedImage.id), icon: <LassoIcon className="w-5 h-5" />, tooltip: t('tooltip.lassoEdit', language) },
    ];
