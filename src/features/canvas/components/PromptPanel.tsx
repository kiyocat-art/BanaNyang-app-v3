...
export const PromptPanel: React.FC<PromptPanelProps> = ({
    ...
    originalImage, modelName, language, mainPanelRef, 
    onNotification,
    ...
    // FIX: Only subscribe to state needed for reactive checks like `canGenerate`.
    const { boardImages } = useCanvasStore();
    const selectedPalette = useGenerationStore(state => state.selectedPalette);
    ...
    const handleDoQueue = (maskImage: File | null = null) => {
        // FIX: Get all state at once to avoid redundancy, stale closures, and fix missing numImages property.
        const {
            ...
            selectedPalette, numPaletteColors, isAutoColorizeSketch, numImages
    ...
    const getGenerationTooltip = () => {
        ...
        if (!originalImage && !(customPrompt && selectedPalette)) return t('error.noOriginalImage', language);
        if (!hasImageTask) return t('error.noOptionsSelected', language);
        return `${t('generateButton', language)} (Ctrl + ↵) - Cost: ${COST_PER_IMAGE} 크레딧`;
    };
