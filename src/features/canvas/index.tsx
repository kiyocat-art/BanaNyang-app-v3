...
const InfiniteCanvas: React.FC<InfiniteCanvasProps> = ({ allHistoryMedia, language, onZoomSelection, onEditSelection, onSaveWorkspace, onSaveWorkspaceAs, onLoadWorkspace, notification, onNotification, isModalOpen, onCopySelection, onPasteFromClipboard, onLoadGenerationParams, saveDirectoryHandle }) => {
    const {
        ...
        downloadSelection,
        updateImage,
        groupEditModeId, setGroupEditModeId, removeImageFromGroup,
    } = useCanvasStore();
    ...
    const actionRingPosition = useMemo(() => {
        if (selectedImageIds.size === 1 && selectedGroupIds.size === 0 && canvasRect && !groupEditModeId) {
        ...
        return null;
    }, [selectedImageIds, selectedGroupIds, boardImages, pan, zoom, canvasRect, groupEditModeId]);
    ...
    const handleMouseDownOnCanvas = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (useCanvasStore.getState().groupEditModeId) {
    ...
    const handleElementMouseDown = (e: React.MouseEvent<HTMLDivElement>, id: string, type: 'image' | 'group') => {
        ...
        setTimeout(() => {
            ...
            const handleMouseUp = () => {
                const { groupEditModeId, selectedImageIds, boardImages, boardGroups } = useCanvasStore.getState();
                if (interactionRef.current?.type === 'drag' && groupEditModeId && selectedImageIds.size > 0) {
                    const group = boardGroups.find(g => g.id === groupEditModeId);
                    if (group) {
                        const draggedImageIds = Array.from(selectedImageIds);
                        for (const imageId of draggedImageIds) {
                            const image = boardImages.find(i => i.id === imageId);
                            if (image) {
                                const imageCenter = { x: image.x + image.width / 2, y: image.y + image.height / 2 };
                                const isOutside = imageCenter.x < group.x || imageCenter.x > group.x + group.width ||
    ...
    const handleContextMenu = (e: React.MouseEvent<HTMLDivElement>) => {
        e.preventDefault();
        e.stopPropagation();
        const handleActualCopy = async () => { try { const blob = await onCopySelection(); if (!blob) return; await navigator.clipboard.write([ new ClipboardItem({ 'image/png': blob }) ]); onNotification(t('copy.success', language), 'success'); } catch (error) { console.error("Failed to copy image to clipboard:", error); onNotification(t('error.copyFailed', language), 'error'); } };
        const items: any[] = [];
        ...
        if (isBackgroundClick) {
            items.push( { label: t('contextMenu.uploadImage', language), onClick: () => fileInputRef.current?.click() }, { label: t('contextMenu.paste', language), onClick: () => onPasteFromClipboard({ x: e.clientX, y: e.clientY }) } );
             items.push({ type: 'separator' });
             items.push( { label: t('contextMenu.saveWorkspace', language), onClick: onSaveWorkspace, disabled: boardImages.length === 0 && boardGroups.length === 0 }, { label: t('contextMenu.saveWorkspaceAs', language), onClick: onSaveWorkspaceAs, disabled: boardImages.length === 0 && boardGroups.length === 0 }, { type: 'separator' }, { label: t('contextMenu.loadWorkspace', language), onClick: () => onLoadWorkspace() } );
        } else {
            if (hasSelection) { items.push({ label: t('contextMenu.copySelection', language), onClick: handleActualCopy }); items.push({ type: 'separator' }); }
            if (selectedImageIds.size > 1 && selectedGroupIds.size === 0) {
                ...
                if (imagesInSelectionAreUngrouped) { items.push({ label: t('contextMenu.groupSelection', language), onClick: groupSelection }); }
            } else if (selectedGroupIds.size > 0) {
                ...
                if (selectedGroupIds.size === 1) { const groupId = Array.from(selectedGroupIds)[0]; items.push({ label: t('contextMenu.renameGroup', language), onClick: () => setEditingGroupId(groupId) }); }
            }
        }
        if (items.length > 0) { setContextMenu({ x: e.clientX, y: e.clientY, onClose: () => setContextMenu(null), items }); }
    };
    
    const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => { e.preventDefault(); const scaleAmount = 1.1; const newZoom = e.deltaY < 0 ? zoom * scaleAmount : zoom / scaleAmount; const clampedZoom = Math.max(0.1, Math.min(5, newZoom)); const rect = canvasRef.current!.getBoundingClientRect(); const mouseX = e.clientX - rect.left; const mouseY = e.clientY - rect.top; const newPanX = mouseX - (mouseX - pan.x) * (clampedZoom / zoom); const newPanY = mouseY - (mouseY - pan.y) * (clampedZoom / zoom); setZoom(() => clampedZoom); setPan(() => ({ x: newPanX, y: newPanY })); };
    
    const cursorClass = isSpacebarDown ? 'cursor-grab' : 'cursor-default';
    const notificationStyles = { base: "absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-full shadow-lg z-[200] animate-fade-in-out-top", success: "bg-yellow-400 text-zinc-800", error: "bg-red-600 text-white", };
    const notificationIcon = notification?.type === 'success' ? <CheckCircleIcon className="w-5 h-5" /> : <ExclamationIcon className="w-5 h-5" />;

    const {pan: currentPan, zoom: currentZoom} = useCanvasStore();
    
    const handleDownload = useCallback(async () => { const count = selectedImageIds.size; if (count === 0) return; await downloadSelection(saveDirectoryHandle); if (saveDirectoryHandle) { onNotification(`${count}개의 파일이 ${saveDirectoryHandle.name} 폴더에 저장되었습니다.`, 'success'); } else { const message = count > 1 ? t('downloadCompleteMultiple', language, { count }) : t('downloadComplete', language); onNotification(message, 'success'); } }, [downloadSelection, saveDirectoryHandle, onNotification, language, selectedImageIds]);
    const handleGroupDoubleClick = (groupId: string) => { setGroupEditModeId(groupId); };
    ...
    return (
        <div className="flex-grow flex flex-col relative min-h-0">
             <CanvasOverlays 
                ...
                language={language}
            />
            <div ref={canvasRef} className={`flex-grow w-full h-full relative bg-zinc-900 overflow-hidden grid-background ${cursorClass}`} onDragOver={(e) => e.preventDefault()} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDrop={handleDrop} onMouseDown={handleMouseDownOnCanvas} onWheel={handleWheel} onContextMenu={handleContextMenu} tabIndex={0} >
                 {notification && ( <div key={notification.id} className={`${notificationStyles.base} ${notificationStyles[notification.type]}`}> {notificationIcon} <span>{notification.message}</span> </div> )}
                 {isDraggingOver && ( <div className="absolute inset-0 bg-sky-500/20 border-4 border-dashed border-sky-500 pointer-events-none z-[100] flex items-center justify-center transition-all duration-200"> <p className="text-2xl font-bold text-white bg-black/50 px-4 py-2 rounded-lg">{t('uploader.orDragAndDrop', language)}</p> </div> )}
                <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: '0 0' }}>
                    {visibleGroups.map(group => ( <CanvasGroup key={group.id} group={group} onMouseDown={(e, id) => handleElementMouseDown(e, id, 'group')} onContextMenu={handleContextMenu} isSuspended={false} onDoubleClick={handleGroupDoubleClick} /> ))}
                    {visibleImages.map(image => ( <CanvasImage key={image.id} image={image} onContextMenu={handleContextMenu} onMouseDown={(e, id) => handleElementMouseDown(e, id, 'image')} /> ))}
                    {marquee && ( <div className="absolute border-2 border-dashed border-white bg-white/20" style={{ left: marquee.x, top: marquee.y, width: marquee.width, height: marquee.height, zIndex: 9998 }} /> )}
                    {selectionBounds && !groupEditModeId && <SelectionBox bounds={selectionBounds} />}
                </div>
                <input type="file" ref={fileInputRef} multiple accept="image/*,.bananyang" className="hidden" onChange={(e) => { if (e.target.files) { const files: File[] = Array.from(e.target.files); const workspaceFile = files.find(f => f.name.endsWith('.bananyang')); if (workspaceFile) { fileToText(workspaceFile).then(content => onLoadWorkspace(content)); } else { handleUploadAndPositionImages(files); } } }} />
            </div>
        </div>
    );
};
...
export const Canvas: React.FC<CanvasProps> = (props) => {
    ...
    return (
        ...
        <PromptPanel customPrompt={props.customPrompt} onCustomPromptChange={props.onCustomPromptChange} onQueueGeneration={props.onQueueGeneration} isProcessing={props.isProcessing} generationQueue={props.generationQueue} originalImage={props.originalImage} modelName={props.modelName} language={props.language} mainPanelRef={props.mainPanelRef} onNotification={props.onNotification} folders={props.folders} onSavePreset={props.onSavePreset} saveFolders={props.saveFolders} />
