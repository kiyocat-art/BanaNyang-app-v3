...
export const RoleIndicator: React.FC<{ role: BoardImage['role'], refIndex?: number, zoom: number }> = ({ role, refIndex, zoom }) => {
    ...
    if (role === 'original') {
        ...
        content = <span className="text-[10px] font-bold text-white leading-none">원본</span>;
        ...
        content = <span className="text-[10px] font-bold text-white leading-none">참조 {refIndex + 1}</span>;
        customClasses = 'w-auto px-1.5 h-5 rounded-md';
        transform = `scale(${1/zoom}) translate(-25%, -25%)`;
    }
    ...
    return (
        <div 
            className={`${commonClasses} ${customClasses} ${!bgColor.startsWith('#') ? bgColor : ''}`}
            style={{ 
                ...(bgColor.startsWith('#') ? { backgroundColor: bgColor } : {}),
                transform: transform,
