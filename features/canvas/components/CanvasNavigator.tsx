...
export const CanvasNavigator: React.FC<{ language: Language; canvasRef: React.RefObject<HTMLElement>; }> = ({ language, canvasRef }) => {
    ...
    const handleZoomToFit = () => {
        ...
        const PADDING = 50;
        const scaleX = (canvasRect.width - PADDING * 2) / contentBounds.width;
        const scaleY = (canvasRect.height - PADDING * 2) / contentBounds.height;
        const newZoom = Math.min(scaleX, scaleY, 5);
        const newPanX = (canvasRect.width - contentBounds.width * newZoom) / 2 - contentBounds.x * newZoom;
        const newPanY = (canvasRect.height - contentBounds.height * newZoom) / 2 - contentBounds.y * newZoom;
        setZoom(() => newZoom);
    ...
    const handleMinimapClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const newPanX = -targetCanvasX * zoom + canvasRect.width / 2;
        const newPanY = -targetCanvasY * zoom + canvasRect.height / 2;
        setPan(() => ({ x: newPanX, y: newPanY }));
    };
    ...
    if (canvasRect && contentBounds.defined && zoom > 0) {
        const contentDisplayWidth = contentBounds.width * minimapScale;
        const contentDisplayHeight = contentBounds.height * minimapScale;
        const contentOffsetX = (MINIMAP_SIZE - contentDisplayWidth) / 2;
        const contentOffsetY = (MINIMAP_SIZE - contentDisplayHeight) / 2;
        viewport = {
            width: (canvasRect.width / zoom) * minimapScale, height: (canvasRect.height / zoom) * minimapScale,
             ...
             {isOpen ? (
                <div className="relative bg-neutral-800/80 backdrop-blur-md border border-neutral-700 rounded-lg p-2 shadow-2xl animate-category-fade-in">
                    ...
                    <div className="flex flex-col items-end gap-2 pt-4">
                        <div ref={minimapRef} className="w-[150px] h-[150px] bg-neutral-900/50 rounded-md overflow-hidden relative cursor-pointer" onClick={handleMinimapClick}>
